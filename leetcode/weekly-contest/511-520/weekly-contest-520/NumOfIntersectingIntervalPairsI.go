...
func NumOfIntersectingIntervalPairsI(intervals [][]int) (r int) {
	for i := 0; i < len(intervals)-1; i++ {
		x, x2 := intervals[i][0], intervals[i][1]
		for j := i + 1; j < len(intervals); j++ {
			y, y2 := intervals[j][0], intervals[j][1]
			maxSide, minSide := max(x, y), min(x2, y2)
			if maxSide <= minSide {
				r++
			}
		}
