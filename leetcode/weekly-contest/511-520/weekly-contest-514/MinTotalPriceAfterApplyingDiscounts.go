...
func MinTotalPriceAfterApplyingDiscounts(prices []int, discounts []int) (r float64) {
	...
	for i := 0; i < len(prices); i++ {
		if i < len(discounts) {
			r += float64(prices[i]) * float64(100-discounts[i]) / float64(100)
		} else {
			r += float64(prices[i])
		}
	}
