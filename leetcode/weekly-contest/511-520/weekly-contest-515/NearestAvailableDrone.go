...
func NearestAvailableDrone(drones [][]int, target []int) (r int) {
	r = -1
	tempVal := -1
	for i := 0; i < len(drones); i++ {
		x, y := abs(drones[i][0]-target[0]), abs(drones[i][1]-target[1])
		if x+y <= drones[i][2] && r == -1 {
			r = i
			tempVal = x + y
		} else if x+y <= drones[i][2] && x+y < tempVal {
			r = i
			tempVal = x + y
		}
	}
